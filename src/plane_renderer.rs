...
impl PlaneRenderer {
    pub fn initializel_content(gl: &gl::Gl) -> PlaneRenderer {
        write_log("arcore_jni::PlaneRenderer::initializel_content");

        let shader_program = util::create_program(gl, VS_SRC, FS_SRC);

        if shader_program == 0 {
            write_log("arcore_jni::PlaneRenderer::initializel_content Could not create program.");
        }
        ...
        gl.tex_image_2d(gl::TEXTURE_2D, 0, gl::RGB as gl::GLint, 2048 as gl::GLsizei,
                        2048 as gl::GLsizei, 0, gl::RGB, gl::UNSIGNED_BYTE, Some(&image));
    ...
    pub fn draw(&mut self, gl: &gl::Gl, projection_mat: ::glm::Mat4, view_mat: ::glm::Mat4, session: *const ArSession, plane: *const ArPlane, color: ::glm::Vec3) {
        write_log("arcore_jni::PlaneRenderer::on_draw");

        if self.shader_program_ == 0 {
            write_log("arcore_jni::PlaneRenderer::on_draw shader_program is null.");
            return;
        }
...
//        self.update_for_plane(session, plane);
        ...
        let mut model_mat = ::glm::mat4(self.model_mat_[0], self.model_mat_[1], self.model_mat_[2], self.model_mat_[3],
                                      ...
                                      self.model_mat_[8], self.model_mat_[9], self.model_mat_[11], self.model_mat_[12],
...
//        if polygon_length == 0 {
//            write_log("arcore_jni::PlaneRenderer::update_for_plane shader_program is null.");
//        }
//
//        let mut vertices_size: usize = (polygon_length / 2) as usize;
//        let raw_vertices: Vec<::glm::Vec2> = Vec::with_capacity(vertices_size);
//        ArPlane_getPolygon(session, plane, raw_vertices.as_void_ptr() as *mut f32);
//
//
...
//        let plane_center: ::glm::Vec2 = ::glm::vec2(self.model_mat_[11], self.model_mat_[14]);
...
//            let result_v: ::glm::Vec2 = scale * d + plane_center;
