...
pub fn load_shader(gl: &Gl, shader_type: GLenum, shader_source: &[u8]) -> GLuint {
    ...
    unsafe {
        ...
        write_log(&format!("servo_jni::util::load_shader : shader = {}", shader));
        ...
        write_log(&format!("servo_jni::util::load_shader : compiled = {}", compiled));

        if compiled == 0 {
            let mut info_len = 0;
            info_len = gl.get_shader_iv(shader, INFO_LOG_LENGTH);

            write_log(&format!("servo_jni::util::load_shader : info_len = {}", info_len));
...
pub fn create_program(gl: &Gl, vertex_source: &[u8], fragment_source: &[u8]) -> GLuint {
    ...
    unsafe {
        ...
        write_log(&format!("servo_jni::util::create_program : vertex_shader = {}", vertex_shader));
        ...
        write_log(&format!("servo_jni::util::create_program : fragment_shader = {}", fragment_shader));
        ...
        write_log(&format!("servo_jni::util::create_program : program = {}", program));
        ...
        if program != 0 {
            ...
            write_log(&format!("servo_jni::util::create_program : link_status = {}", link_status));
